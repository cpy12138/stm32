...
//数字显示(前面的0不会去掉)
void LCD_ShowNum0(u16 x,u16 y,u32 num,u8 len,u8 size)
{         	
	u8 t,temp;
	u8 enshow=0;						   
	for(t=0;t<len;t++)
	{
		...
		if(enshow==0&&t<(len-1))
		{
			if(temp==0)
			{
				LCD_ShowChar(x+(size/2)*t,y,POINT_COLOR,BACK_COLOR,'0',size,1);
				continue;
			}else enshow=1; 
...
//数字显示(小数)
//void LCD_ShowFloatNum(u16 x,u16 y,float num,u8 len,u8 size)
//{
//	float temp;
//	LCD_ShowNum(x,y,(int)num,1,12);
//	Show_Str(x+5,y-4,BLACK,WHITE,".",2,1);
//	temp = (num - (int)num)*100.0;
//	LCD_ShowNum0(x+9,y,temp,2,12);
//}
//******************************************************************
//函数名：  GUI_DrawFont16
//作者：    xiao冯@全动电子
...
void Gui_Drawbmp16(u16 x,u16 y,const unsigned char *p) //显示40*40 QQ图片
{
  int i; 
	...
	LCD_SetWindows(x,y,x+127-1,y+128-1);//窗口设置
    for(i=0;i<127*128;i++)
	{	
	 	picL=*(p+i*2);	//数据低位在前
		picH=*(p+i*2+1);				
