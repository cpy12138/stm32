...
{
	while((SPIx->SR&((uint16_t)0x0002))==RESET);		//等待发送区空	  
	SPIx->DR=Byte;	 	//发送一个byte   
	while((SPIx->SR&((uint16_t)0x0001))==RESET);//等待接收完一个byte  
...
void SPI_SetSpeed(SPI_TypeDef* SPIx,u8 SpeedSet)
{
	...
	if(SpeedSet==1)//高速
	{
		SPIx->CR1|=((uint16_t)0x0000);//Fsck=Fpclk/2	
	}
	else//低速
	{
		SPIx->CR1|=((uint16_t)0x0020); //Fsck=Fpclk/32
	}
